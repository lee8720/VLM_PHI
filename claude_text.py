...
# Updated order of message prompts
messages_list = [
    # Originally Prompt 3, now Prompt 1
    """
    You will be provided with text extracted from an image using Tesseract OCR. The task is to review this text and confirm whether all personal information related to the patient has been fully de-identified. Keep in mind that the image may not contain any text at all.
    ...
    # Originally Prompt 1, now Prompt 2
    """
    You will be provided with text extracted from an image using Tesseract OCR. The task is to review this text and confirm whether all personal information related to the patient has been fully de-identified. Keep in mind that the image may not contain any text at all.

    After completing the analysis, create a table with three columns: 
    1. The first column should list the categories of personal information detected in the text (with no limitation on the number or type of categories). 
    2. The second column should indicate whether each category is present in the text ('Yes' or 'No'). 
    ...
    # Originally Prompt 2, now Prompt 3
    """
    You will be provided with text extracted from an image using Tesseract OCR. The task is to review this text and confirm whether all personal information related to the patient has been fully de-identified. Personal information is defined by the following five categories: 1. Patient Name; 2. Patient ID; 3. Patient Date of Birth; 4. Patient Age; 5. Patient Gender. Keep in mind that the image may not contain any text at all.
...
for r in range(num_repetition):
    for file_name, txt in zip(file_names, texts):
        for idx, message in enumerate(messages_list):
            name_without_extension = int(os.path.splitext(file_name)[0])
            retries = 0
            success = False
            ...
            while retries < max_retries and not success:
                try:
                    response = client.messages.create(model="claude-3-haiku-20240307", max_tokens=1000, temperature=0.0,
                                                      messages=[
                                                          {
                                                              "role": "user",
                                                              "content": [
